(* 7_microgpt_batched.ml — Stage 7: Batched Training with BLAS dgemm
   ================================================================

   Based on Stage 6 (BLAS-accelerated matmul, 4 layers, 64-dim, 207K params).

   THE KEY INSIGHT
   ===============
   Stage 6 was slow (65s) because we fed tokens one at a time:
     64 positions × 25 matmuls each = 1600 tiny BLAS dgemv calls per step.
   Each call was [64,64] @ [64,1] — BLAS can barely warm up its SIMD pipeline.

   Stage 7 feeds ALL positions at once:
     25 BLAS dgemm calls per step, each [64,64] @ [64,64] or similar.
   BLAS gets full [S,N] @ [N,M] matrix-matrix multiplies where it excels.

   WHAT CHANGES
   ============
   Training path (batched):
     - batch_embed: look up all token embeddings → [S, 64] matrix
     - batch_matmul: W[m,n] × X[S,n] → Y[S,m] in one BLAS dgemm
     - batch_rmsnorm: normalize each row (position) independently
     - batch_fused_attention: causal mask attention on all positions at once
     - tensor_gelu/tensor_add work unchanged (element-wise on flat arrays)

   Inference path (single-token, unchanged):
     - Keeps KV cache for autoregressive generation
     - Same code as Stage 6

   BLAS OPS (updated from Stage 6)
   ================================
   op=0: C = A @ B         op=1: C += A @ B
   op=2: C = A @ B^T       op=3: C += A @ B^T
   op=4: C = A^T @ B       op=5: C += A^T @ B
   Encoded as 3-bit flag: bit2=transA, bit1=transB, bit0=accumulate.

   Compile:
     ocamlopt -O2 -o microgpt_batched \
       blas_stubs.c 7_microgpt_batched.ml \
...
(* ══════════════════════════════════════════════════════════════════════
   UTILITIES (unchanged from Stage 6)
   ══════════════════════════════════════════════════════════════════════ *)
...
(* ══════════════════════════════════════════════════════════════════════
   SHARED OPS — work for both batched and single-vector paths
   (element-wise operations don't care about shape)
   ══════════════════════════════════════════════════════════════════════ *)
...
(* GeLU: element-wise, works on any flat array regardless of shape. *)
let tensor_gelu x =
  let n = Array.length x.data in
...
(* Extract row from 2D tensor [rows, cols] -> [cols].
   Used for: embedding lookup (single token), logits extraction. *)
...
(* ══════════════════════════════════════════════════════════════════════
   SINGLE-VECTOR OPS — used by inference (one token at a time)
   ══════════════════════════════════════════════════════════════════════ *)

(* Single-vector matmul: W[m,n] @ x[n] -> y[m].
   Uses BLAS but as dgemv (n_cols=1). Kept for inference path. *)
let matmul w x =
  ...
  let backward () =
    (* dW[m,n] += dy[m,1] @ x[1,n] — outer product, op=1 NN accum *)
    blas_dgemm 1 m n 1 y_grad x.data w.grad;
    (* dx[n] += W^T @ dy — op=5 TN accum *)
...
(* ══════════════════════════════════════════════════════════════════════
   BATCHED OPS — used by training (all positions at once)

   Convention: "batched" tensors are flat float arrays of size [S * dim]
   where S = number of positions (sequence length). The row dimension
   (S) is inferred from Array.length / dim, not from shape metadata.
   This lets us use tensor_add and tensor_gelu without modification.
   ══════════════════════════════════════════════════════════════════════ *)

(* Batched matmul: Y[S, m] = X[S, n] @ W[m, n]^T
   One BLAS dgemm replaces S individual matrix-vector multiplies.
   This is THE optimization that makes Stage 7 fast.

   Forward:  op=2 (NT overwrite): C[S,m] = X[S,n] @ W[m,n]^T
   Backward: op=5 (TN accum): dW[m,n] += dY[S,m]^T @ X[S,n]
             op=1 (NN accum): dX[S,n] += dY[S,m] @ W[m,n] *)
...
(* Batched RMSNorm: normalize each row of [S, dim] independently.
   Uses n_embd as the row width (all our normalized tensors are [S, n_embd]). *)
...
(* Batched embedding: look up all tokens at once → [S, n_embd] *)
let batch_embed wte tokens n_embd =
  ...
  for i = 0 to s - 1 do
    ...
    let dst = i * n_embd in
    Array.blit wte.data src data dst n_embd
  done;
...
(* ══════════════════════════════════════════════════════════════════════
   MODEL DEFINITION — identical weights to Stage 5/6
   ══════════════════════════════════════════════════════════════════════ *)
...
(* Batched multi-head attention with RoPE and causal mask.

   Input: Q, K, V are autograd nodes from batch_matmul, each [S, n_embd].
   Output: attention output [S, n_embd].

   The attention inner loops (score computation, softmax, value weighting)
   use scalar loops. BLAS wins come from the batch_matmul for Wq/Wk/Wv/Wo
   that produce Q/K/V — those are the large matrix-matrix multiplies. *)
let batch_fused_attention q k v seq_len =
  ...
  (* Attention weights: [n_head][seq_len * seq_len], stored for backward *)
  ...
  for h = 0 to n_head - 1 do
    ...
    for i = 0 to seq_len - 1 do
      (* Score: q_rot[i] · k_rot[j] for j ≤ i (causal mask) *)
      let max_s = ref neg_infinity in
      for j = 0 to i do
        let s = ref 0.0 in
        for d = 0 to head_dim - 1 do
          s := !s +. q_rot.(i * n_embd + h_off + d)
                     *. k_rot.(j * n_embd + h_off + d)
        done;
        let score = !s *. scale in
        wt.(i * seq_len + j) <- score;
        if score > !max_s then max_s := score
      done;
      (* Causal softmax: only over positions 0..i *)
      let sum_exp = ref 0.0 in
      for j = 0 to i do
      ...
      for d = 0 to head_dim - 1 do
        ...
        for j = 0 to i do
          s := !s +. wt.(i * seq_len + j)
                     *. v.data.(j * n_embd + h_off + d)
        done;
        out_data.(i * n_embd + h_off + d) <- !s
      done
    done
  done;
  ...
  let backward () =
    (* Accumulate gradients in rotated space, then inverse-rotate *)
    let dq_rot = Array.make (seq_len * n_embd) 0.0 in
    let dk_rot = Array.make (seq_len * n_embd) 0.0 in
    let dw_buf = Array.create_float seq_len in
    for h = 0 to n_head - 1 do
      let h_off = h * head_dim in
      let wt = all_weights.(h) in
      for i = 0 to seq_len - 1 do
        (* dweight[j] and accumulate dv *)
        for j = 0 to i do
          let s = ref 0.0 in
          for d = 0 to head_dim - 1 do
            let dy = out_grad.(i * n_embd + h_off + d) in
            s := !s +. dy *. v.data.(j * n_embd + h_off + d);
            v.grad.(j * n_embd + h_off + d) <-
              v.grad.(j * n_embd + h_off + d)
              +. wt.(i * seq_len + j) *. dy
          done;
          dw_buf.(j) <- !s
        ...
        let dot = ref 0.0 in
        for j = 0 to i do
          dot := !dot +. dw_buf.(j) *. wt.(i * seq_len + j)
        done;
        (* Score backward → dq_rot[i], dk_rot[j] *)
        for j = 0 to i do
          let ds = wt.(i * seq_len + j) *. (dw_buf.(j) -. !dot) *. scale in
          for d = 0 to head_dim - 1 do
            dq_rot.(i * n_embd + h_off + d) <-
              dq_rot.(i * n_embd + h_off + d)
              +. ds *. k_rot.(j * n_embd + h_off + d);
            dk_rot.(j * n_embd + h_off + d) <-
              dk_rot.(j * n_embd + h_off + d)
              +. ds *. q_rot.(i * n_embd + h_off + d)
          done
        done
      done
    done;
...
(* Batched forward: embed all tokens → transformer layers → logits *)
...
(* ══════════════════════════════════════════════════════════════════════
   INFERENCE FORWARD — single-token with KV cache (unchanged from Stage 6)
   ══════════════════════════════════════════════════════════════════════ *)
...
let fused_multi_head_attention x layer kv =
  ...
  (* No backward needed for inference, but keep structure for compatibility *)
...
(* Batched loss: run all positions through the model at once,
   then extract per-position logits for softmax + NLL. *)
...
(* ══════════════════════════════════════════════════════════════════════
   INFERENCE (unchanged — single-token with KV cache)
   ══════════════════════════════════════════════════════════════════════ *)
