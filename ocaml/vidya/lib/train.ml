...
(* One Adam step with fixed learning rate (no schedule).
   Used for interactive RL fine-tuning at low LR. *)
...
(* Optional params for crash recovery:
     ~start_step    — resume from this step (default 0)
     ~checkpoint_base — save intermediate checkpoints every 10K steps
                        as "{base}_{step}k.bin" (default "" = no intermediates) *)
...
let train model params tokenized_docs num_steps
  ...
  for step = start_step to num_steps - 1 do
    ...
    (* Intermediate checkpoints every 10K steps for crash recovery.
       Only saves if checkpoint_base is set and we're not on the last step
       (final checkpoint is saved by the caller). *)
    if checkpoint_base <> "" && (step + 1) mod 10000 = 0
