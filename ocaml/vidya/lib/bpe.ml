...
(* encode: Tokenize a string using learned BPE merges.
   ...
   (= learned first = most frequent). This ensures deterministic
   tokenization matching the training distribution. *)
let encode tok doc =
  let n = String.length doc in
  if n = 0 then [|tok.bos_id; tok.bos_id|]
  else begin
    let tokens = Array.init n (fun i ->
      Hashtbl.find tok.char_to_id (String.make 1 doc.[i])
    ) in
    let len = ref n in
    ...
    let result = Array.make (!len + 2) tok.bos_id in
