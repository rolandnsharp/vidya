(* generate.ml — Text generation with symbolic constraints
   ...
   Both apply Symbolic.apply to logits before sampling, enforcing
   repetition penalty and word validation constraints. *)

(* sample: Generate text from scratch (starting from BOS).
   Feeds BOS, then autoregressively samples until BOS is produced
   again or block_size is reached. Symbolic constraints are applied
   to every sampling step. *)
let sample model docs vocab bos_id temperature =
  let sym_ctx = Symbolic.create vocab docs bos_id in
  ...
  while !pos < Model.block_size do
    ...
    (* Apply symbolic constraints, then temperature scaling + softmax *)
...
let prompted model docs vocab tok bos_id prompt temperature =
  let sym_ctx = Symbolic.create vocab docs bos_id in
  let kv_caches = Array.init Model.n_layer (fun _ -> Model.make_kv_cache ()) in
