(* ten_microgpt_scaled.ml — Stage 10: Scaled Up
   ================================================================

   Based on Stage 9 (BPE tokenizer, 53s).

   WHAT CHANGES
   ============
   Everything we built — BLAS, batching, BPE — now gets real capacity.
   Stage 9 at 64-dim/4-layer/232K params produced recognizable Plotinus
   but with garbled words. The model couldn't distinguish 580 BPE tokens
   in a 64-dim embedding space.

   Scale-up:
     n_embd:     64 → 128   (2x embedding dimension)
     n_head:      4 → 8     (2x attention heads, head_dim stays 16)
     n_layer:     4 → 6     (50% more depth)
     block_size: 64 → 128   (2x context window)
     n_merges:  200 → 500   (128-dim can handle full 580 vocab)
     params:   232K → ~1.3M (5.7x more capacity)

   BLAS benefits more at 128×128 matrices — closer to its sweet spot
   for SIMD utilization and cache blocking.

   Learning rate lowered from 0.003 to 0.001 for the larger model,
   with longer warmup (200 steps).

   Compile:
     ocamlopt -O2 -o microgpt_scaled \
       blas_stubs.c ten_microgpt_scaled.ml \
...
(* ══════════════════════════════════════════════════════════════════════
   MODEL DEFINITION — scaled up from Stage 9
   128-dim, 8 heads, 6 layers, 128-token context, ~1.3M params
   ══════════════════════════════════════════════════════════════════════ *)
...
let () =
  ...
  train model params tokenized_docs 10000;
